...
public class APSKolokvium2021Termin1 {
	...
	public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
		...
		SLLNode<Card> maxNode = firstFriendCards.getFirst();
		while(tmp != null) {
			if(tmp.element.damage() > maxPower1) {
				maxPower1 = tmp.element.damage();
				maxNode = tmp;
			}
			tmp = tmp.succ;
		}
		...
		int n = 0;
		SLLNode<Card> middle = secondFriendCards.getFirst();
		while(tmp2 != null) {
			n++;
			tmp2 = tmp2.succ;
		}
		...
		firstFriendCards.delete(maxNode);
		secondFriendCards.insertAfter(maxNode.element, middle);
